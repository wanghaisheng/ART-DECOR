...
(:let $toplevels := tokenize('404684003','\s'):)
...
let $result :=
   if ($validSearch and not(matches($searchString,'^[0-9]+'))) then
      if (string-length($toplevels[1])>0) then
         collection(concat($get:strTerminologyData,'/snomed-data/descriptions'))//description[ft:query(.,$query,$options)][@topLevelId=$toplevels]
      else(
         collection(concat($get:strTerminologyData,'/snomed-data/descriptions'))//description[ft:query(.,$query,$options)]
      )
   else if ($validSearch and matches($searchString,'^[0-9]+')) then
      collection(concat($get:strTerminologyData,'/snomed-data/descriptions'))//description[@conceptId=$searchString]
